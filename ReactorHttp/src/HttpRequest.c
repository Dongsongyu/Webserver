/*************************************************************************
	> File Name: src/HttpRequest.cpp
	> Author: 
	> Mail: 
	> Created Time: Wed 08 Jan 2025 05:07:13 PM CST
 ************************************************************************/
...
char* splitRequestLine(const char* start, const char* end, const char* sub, char** ptr) {
	char* space = end;
	if (sub != NULL) {
		char* space = memmem(start, end - start, sub, strlen(sub));
		assert(space != NULL);
	}
...
bool parseHttpRequestHeader(struct HttpRequest* request, struct Buffer* readBuf) {
	...
	if (end != NULL) {
		...
		if (middle != NULL) {
			char* key = malloc(middle - start + 1); // 在后面补充一个\0
			strncpy(key, start, middle - start);
			key[middle - start] = '\0';

			char* value = malloc(end - middle - 2 + 1); // 在后面补充一个\0
...
bool parseHttpRequest(struct HttpRequest* request, struct Buffer* readBuf, struct HttpResponse* response, struct Buffer* sendBuf, int socket) {
	...
	while (request->curState != ParseReqDone) {
		...
		if (request->curState == ParseReqDone) {
			// 根据解析出的原始数据,对客户端请求做出处理
			processHttpRequest(request, response);
			// 组织响应数据并发给客户端
			httpResponsePrepareMsg(response, sendBuf, socket);
		}
	}
	request->curState = ParseReqLine; //状态还原,保证还能继续处理第二条及以后的处理
	return flag;
}
...
bool processHttpRequest(struct HttpRequest* request, struct HttpResponse* response) {
	...
	if (S_ISDIR(st.st_mode)) {
		...
		httpResponseAddHeader(response, "Content-type", tmp);
		response->sendDataFunc = sendFile;
